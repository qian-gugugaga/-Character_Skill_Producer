...
description: |
  ...
  依赖 Python + 网页搜索能力；萌娘百科优先走通用 MediaWiki API 脚本，WebFetch 仅作失败降级。
---
...
本 skill 依赖 Python + 网页搜索能力。Python 用于优先调用萌娘百科 MediaWiki API；网页搜索 skill 用于 Wikipedia、Fandom Wiki、Bangumi、Bilibili 等来源，以及萌娘百科 API 不可用时的降级补强。若本 skill 是从 `examples/csp/` 或 `.claude/skills/csp/` 独立安装的，使用当前 skill 目录内的 `scripts/moegirl_api.py`。
...
1. **简介确认**：`python scripts/moegirl_api.py "<角色名或条目名>" --intro`
2. **全文提取**：简介不足时运行 `python scripts/moegirl_api.py "<角色名或条目名>" --full`
3. **候选搜索**：标题不确定、别名、外文名或消歧义时运行 `python scripts/moegirl_api.py "<查询词>" --search`
4. **wikitext 降级**：`extracts` 为空或缺少关键段落时运行 `python scripts/moegirl_api.py "<条目名>" --wikitext`；若站点返回 `action-notallowed`，记录该限制并改用 `--full` 纯文本或其他来源补强
5. **外部降级**：API 不可用时，才使用 WebSearch/WebFetch/其他 wiki，并在调研文件中记录 API 失败原因、已尝试命令、替代来源和可信度影响。
...
1. **萌娘百科 API 优先**：涉及萌娘百科时，先用 `scripts/moegirl_api.py` 的 `--intro`、`--full`、`--search`、`--wikitext`，不要默认 WebFetch 直抓页面。
2. **常规搜索**：用可用的网页搜索 skill 获取原始页面、条目名、别名和相关作品名。
3. **多搜索引擎复查**：常规搜索失败或结果污染时，改用 `multi-search-engine`，分别尝试中文名、日文名、英文名、作品名 + 角色名、站内搜索语法。
4. **站点替代入口**：
...
5. **用户资料兜底**：关键维度仍缺失时，暂停询问用户是否有设定集、访谈、BD 特典、字幕、截图、视频链接或本地文本。
6. **失败记录**：最终仍失败时，在对应 `references/research/0X-*.md` 写明失败站点、失败原因、已尝试方法、替代来源和可信度，不允许只写「没搜到」。
...
1. **确认角色**：用户指定要生成的虚构角色（动画/游戏/小说/影视中的任意角色）
2. **深度调研**：从萌娘百科、Wikipedia、Fandom Wiki、Bangumi、Bilibili 等 10+ 来源搜集角色资料
3. **知识蒸馏**：从原始资料中提炼角色的 5 个核心行为模式、完整表达质感、社会认知和决策逻辑
4. **生成文件**：按标准格式输出 SKILL.md + references/research/ 下 5 个调研文件
5. **注册部署**：将生成的 skill 复制到 `.claude/skills/` 目录使其可用
   ...
   ### 必须覆盖的维度
   1. **设定与世界观** — 作品背景、角色基本设定、所属组织/团体、生活场景
   2. **人格分析** — 表层人格、深层人格、核心创伤、成长弧线、制作组/评论界视角
   3. **表达质感** — 原语言语言特征（自称、敬语、句式、词汇、口头禅）、目标语言适配、非语言 tells、语速节奏、经典台词
   4. **人际关系** — 与每个重要人物的关系性质、视角、关键互动、关系状态
   5. **关键场景** — 不少于 8 个场景的详细还原（情境→内心→言行→意义）
...
搜索方向：
- 先运行 `python scripts/moegirl_api.py "[角色名]" --intro` 确认萌娘百科条目；简介不足时运行 `--full`，标题不确定时运行 `--search`，extract 缺关键段落时运行 `--wikitext`
- 萌娘百科性格段落或全文中的行为描述
- 角色深度分析文章
- 寻找具体行为描述而非性格标签
- 角色在不同情境下的行为差异
...
- 每条信息标注来源URL和可信度
- 记录萌娘百科 API 使用的命令、resolved_title、page_url；若失败，记录错误、替代来源和可信度影响
- 区分「官方设定」vs「粉丝解读」
- 发现矛盾直接记录，不要调和
...
- **信息源清单更新**：对于 BanG Dream 角色，增加 `bandori.fandom.com`（含游戏剧情摘要）、`bestdori.com`（卡面/活动数据库）、游戏内活动剧情翻译
- **诚实边界需注明**：调研覆盖了哪些作品/媒体的内容，哪些未覆盖（如仅覆盖动画未覆盖游戏、或仅覆盖主线未覆盖活动剧情）
...
CSP 生成的 Skill 文件本质上是**角色扮演的配置指令**，不是文学作品。因此在 Skill 正文本身（身份卡、行为描述）中，应以自然的口语节奏书写——参考 [Humanizer](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing) 的 AI 文本检测方法论，主动避开：
...
- 「在……下」「当……时」等书面句式——口语不这样开头
  ...
  * 关键引用标注出处（话数/章节）
  ...
  1. 在 `examples/<name>/` 创建源文件
  2. 复制到 `.claude/skills/<name>/` 使其注册为可用 skill
