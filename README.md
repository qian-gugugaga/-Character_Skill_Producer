...
### Stop writing character cards. Build characters that can react.
...
**CSP 是一个二次元角色 Skill 生产器。**

给它一个角色名和作品名，它会调研资料、交叉验证、蒸馏行为模式，最后生成一个可以直接安装、直接对话、带资料日期的 Agent Skill。

不是角色卡。不是设定集。不是「她很傲娇」「他说话很中二」这种标签堆砌。

CSP 要做的是：让 AI 知道角色在新情境里**会怎么反应、怎么说话、怎么误解别人、怎么保护自己、什么时候该承认不知道**。

[看效果](#效果示例) · [为什么不是角色卡](#为什么不是角色卡) · [安装](#安装) · [工作原理](#工作原理) · [资料时间边界](#资料时间边界与更新) · [预置角色](#预置角色)
...
```bash
python scripts/source_search.py "八幡海铃" --work "BanG Dream!" --mode discover
python scripts/source_search.py "芙宁娜" --work "原神" --sources moegirl
python scripts/moegirl_api.py "八幡海铃" --search
python scripts/moegirl_api.py "八幡海铃" --full
```
